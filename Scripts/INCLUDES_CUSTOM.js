...
function setParcelDistrict(pCapId)
{
  var myParcel;
      var capParcelResult = aa.parcel.getParcelandAttribute(pCapId,null);
      if (capParcelResult.getSuccess())
         {
             ...
             {
                myParcel = Parcels[zz];
                 logDebug("Adding Condition to parcel #" + zz + " = " + myParcel.getParcelNumber())
                 aa.print(myParcel.getParcelNumber()+' '+ myParcel.getInspectionDistrict());
                 aa.parcel.addParceDistrictForDaily(pCapId.getID1(), pCapId.getID2(), pCapId.getID3(), myParcel.getParcelNumber(), myParcel.getInspectionDistrict());
             }   
           }
}
...
function emailContact(mSubj,mText)   // optional: Contact Type, default Applicant
	{
	var replyTo = "donotreply@lwdd.net";
...
function emailContactRDK(e,t)
{
	var a="donotreply@lwdd.net";
	var r="ALL";
	var s="";
	...
	var n = aa.people.getCapContactByCapID(capId);
	if (n.getSuccess())
	{
		var i = n.getOutput();
		var contactType = "";
		for(yy in i)
		{
			contactType = i[yy].getCapContactModel().getPeople().getContactType();
			if (r.equals("ALL") || (!r.equals("ALL") && r.equals()))
			{
				s=""+i[yy].getEmail();
				if (s.indexOf("@")>0)
				{
					aa.sendMail(a,s,"",e,t);
					logDebug("Successfully sent email to "+contactType+" - "+s);
					//aa.sendMail("noreply@lwdd.net","wackow@hotmail.com","","subjsct here","text here");
				}
				else
				{
					logDebug("Couldn't send email to "+r+", no valid email address");
				}				
			}
		}
