...
try {	
	...
	if (getResult.getSuccess()) 
	{
		...
		var message = "RECORDID" + "," + "ACCELAID" + "," + "PARCEL_NUMBER" +"," + "APPSTATUS" +'\n';
        ...
        for (var I in list) 
		{
		    if (!list[I].getCapID().getCustomID().contains("EST"))
	        {
				...
				message+= list[I].getCapID().getCustomID().toString() + "," + theCapID + "," + cleanParcel + "," + list[I].getCapStatus().toString() + '\n';
	...
	aa.util.deleteFile(theDirectoryName + "/ACCELA_RW_INTERESTS_INFO.CSV");
	fileObj = aa.util.writeToFile(message, theDirectoryName + "/ACCELA_RW_INTERESTS_INFO.CSV");
	if (fileObj) 
	{
